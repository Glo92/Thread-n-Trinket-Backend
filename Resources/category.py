...
class CategoryResource(Resource):
    def post(self):
        ...
        try:
            ...
            db.session.commit()
            return Response(response=json.dumps(category.serialize()), status=201, mimetype='application/json')
...
class CategoryListResource(Resource):
    def get(self):
        try:
            ...
            response_data = [category.serialize() for category in categories]
...
class CategoryUpdateResource(Resource):
    def put(self, category_id):
        ...
        try:
            ...
            db.session.commit()
            return Response(response=json.dumps(category.serialize()), status=200, mimetype='application/json')
        except Exception as e:
            db.session.rollback()
...
class CategoryDeleteResource(Resource):
    def delete(self, category_id):
        try:
            ...
            db.session.commit()
            return Response(response=json.dumps({'message': 'Category deleted successfully'}), status=204, mimetype='application/json')
        except Exception as e:
            db.session.rollback()
